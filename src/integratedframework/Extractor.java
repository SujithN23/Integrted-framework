...
public class Extractor {
    ...
    public void getArticles() {
        links.forEach(x -> {
            ...
            try {
                ...
                Elements articleLinks = document.select("h2 a[href^=\"http://www.guru99.com/\"]");
                for (Element article : articleLinks) {
                    //Only retrieve the titles of the articles that contain Java 8
                    if (article.text().matches("^.*?(bigdata|cloud|oracle).*$")) {
                        //Remove the comment from the line below if you want to see it running on your editor, 
    ...
    public static void main(String[] args) {
        ...
        bwc.getPageLinks("http://www.guru99.com");
        bwc.getArticles();
        bwc.writeToFile("BigData");
    }
}
